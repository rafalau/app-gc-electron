...
import type { TbsAuctionOption } from '@renderer/types/importacao'

const props = defineProps<{
  aberto: boolean
  leiloes: TbsAuctionOption[]
  loading: boolean
  importing: boolean
...
const emit = defineEmits<{
  (e: 'fechar'): void
  (e: 'select', auctionId: number | null): void
  (e: 'importar'): void
}>()
...
<template>
  <BaseModal :aberto="aberto" titulo="Importar da TBS" @fechar="emit('fechar')">
    <div class="space-y-5">
      <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
        Selecione um leilão ativo para importar os animais direto da API.
      </div>
